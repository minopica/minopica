            ...
            cy.task('loadToken').then((jwt) => {
                ...
                cy.task('loadArrayContracts').then((contratti)=> {
                    ...
                    for (let i=0;i<bodyRequest_bills.cdfList.length;i++) {
                        ...
                        const requestObj = {
                            method: 'GET',
                            url: Cypress.env("base_url_bs") + '/api/v0/ch/payment/retrieve-mdp',
                            headers: {
                                Authorization: 'Bearer ' + `${token}`,
                                'Customer-Id': `${codice_cliente}`,
                                'X-Wind-Client': 'app-ios',
            ...
            cy.task('loadToken').then((jwt) => {
                ...
                cy.task('loadArrayContracts').then((contratti)=> {
                    ...
                    for (let i=0;i<bodyRequest_billingSummary.length;i++) {
                        const requestObj = {
                            method: 'POST',
                            url: Cypress.env("base_url_bs") + '/api/v0/ch-payment/billing/summary',
                            headers: {
                                Authorization: 'Bearer ' + `${token}`,
            ...
            cy.task('loadToken').then((jwt) => {
                ...
                cy.task('loadArrayBills').then((bills)=> {
                    ...
                    for (let i=0;i<bills.length;i++) {
                        ...
                        const requestObj = {
                            method: 'GET',
                            url: Cypress.env("base_url_bs") + '/api/v0/ch/payment/bills/detail',
                            headers: {
                                Authorization: 'Bearer ' + `${token}`,
                                'Customer-Id': `${codice_cliente}`,
                                'X-Wind-Client': 'app-ios',
